...
def _purge_object(kind, pk, delete_interfaces):
    ...
    result = {'name': obj.name, 'kind': kind,
              'services': 0, 'ips': 0, 'vlans': 0, 'vlans_skipped': 0, 'interfaces': 0}
    ...
    if iface_ids:
        ...
        for vlan_id in (tagged_ids | untagged_ids):
            if kind == 'vm':
                other = (
            ...
            if other:
                ...
                Prefix.objects.filter(vlan_id=vlan_id).update(vlan=None)
    ...
    logger.info(
        ...
        f'{result["ips"]} IPs, {result["vlans"]} VLANs '
        f'({result["vlans_skipped"]} ignorés), {result["interfaces"]} interfaces'
    )
    return result
...
class PurgeView(View):
    ...
    def get(self, request):
        devices = Device.objects.select_related('site', 'role', 'device_type', 'tenant').order_by('name')
        vms     = VirtualMachine.objects.select_related('site', 'role', 'tenant').order_by('name')
        return render(request, 'netbox_device_cleaner/purge.html', {'devices': devices, 'vms': vms})
